...
client.on("guildCreate", guild => {
  ...
  .setDescription('The Difference Yeni bir sunucuya eklendi !!')
  ...
  .setColor('RANDOM')
  client.channels.get('503670806682009601').sendMessage(embed)
  client.users.get('282144942619492352').send(embed)
})

client.on("guildDelete", guild => {
   const embed = new Discord.RichEmbed()
  .setDescription('The Difference Bir sunucudan çıkarıldı !!')
  .addField('• Sunucu', `Sunucu adı: ${guild.name}\nSunucu id: ${guild.id}\nSunucu üye sayısı: ${guild.members.size}`)
  .addField('• Sunucu sahibi' , `${guild.owner.user.username}#${guild.owner.user.discriminator}\nID: ${guild.owner.user.id}`)
  .setColor('RANDOM')
  client.channels.get('503670806682009601').sendMessage(embed)
...
if (veri === true) {
  ...
  if (member.guild.channels.get(kanalveri).type === "text") {
    ...
    await bg.print(font, 430, 170, member.user.tag);
    await userimg.resize(282, 282);
    await bg.composite(userimg, 43, 26).write("https://cdn.discordapp.com/attachments/485336791847665664/503301659619885067/guildRemove.png");
      setTimeout(function () {
        member.guild.channels.get(kanalveri).send(new Discord.Attachment("https://cdn.discordapp.com/attachments/485336791847665664/503301659619885067/guildRemove.png"));
      }, 1000);
      setTimeout(function () {
      fs.unlink("./img/" + randname + ".png");
      }, 10000);
  ...
  client.on('message', msg => {
    ...
    if (regex.test(msg.content)==true) {
      if (!msg.member.hasPermission("ADMINISTRATOR")) {
        ...
        return msg.reply('Bu sunucuda linkler **Şakir** tarafından engellenmektedir.Link atmana izin vermeyeceğim!').then(msg => msg.delete(3000));
      } else {
        return;
      };
    } else {
...
client.on('message', msg => {
  if (msg.content === 'sa') {
    msg.react(":regional_indicator_a:").then(() => msg.react(":regional_indicator_a:"))
    msg.reply('Aleyküm selam. Hoşgeldiniz!');
  }
});
  ...
  client.on('guildMemberAdd', async member => {
      ...
      if (member.guild.channels.get(logCh).type === "text") {
        ...
        .setTitle('Hoşgeldin :)')
        .setDescription(`<@${member.user.id}> Hoşgeldin Benim Güzel Kardeşim `)
...
client.on('ready', () => {
  ...
  client.setInterval(() => {
      client.user.setActivity("t!help | Yardım Menüsü Güncellendi.", { type: "WATCHING" });
      client.user.setActivity("t!tavsiye | Tavsiye Gönderebilirsiniz.", { type: "WATCHING" });
      client.user.setActivity("t!oynat | Şarkı Dinleyebilirsiniz!", { type: "WATCHING" });
      client.user.setActivity("t!canlı-destek | Canlı şekilde temsilcimizle konuşabilirsiniz.", { type: "WATCHING" });
  }, 10000);
});
