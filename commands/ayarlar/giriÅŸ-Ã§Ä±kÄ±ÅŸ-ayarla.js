...
module.exports = class BlacklistUserCommand extends Command {
	...
	async run(message, args) {
			...
			if (vt === args.kanal.id) {
				...
				message.channel.send(`:no_entry: Giriş çıkış kanalı zaten **<#${args.kanal.id}>** kanalı olarak ayarlı.`);
			} else {
				this.client.provider.set(message.guild.id, 'girisCikis', args.kanal.id);
				this.client.provider.set(message.guild.id, 'girisCikisK', true);
				return message.channel.send(`:white_check_mark: Giriş çıkış kanalı **<#${args.kanal.id}>** kanalı olarak ayarlandı.`);
			}
	}
};